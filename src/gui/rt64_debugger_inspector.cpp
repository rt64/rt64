                            ...
                            if (headerOpen) {
                                ...
                                for (uint32_t t = 0; t < callDesc.tileCount; t++) {
                                    ...
                                    if (ImGui::TreeNode("Texture tile", "Texture tile #%d:", t)) {
                                        ...
                                        bool dumpRGBA32 = false;// ImGui::Button("Dump RGBA32");
                                        bool dumpTMEM = ImGui::Button("Dump TMEM");
                                        uint32_t textureIndex = 0;
                                        const Texture *texture = nullptr;
                                        if (dumpRGBA32 || dumpTMEM) {
                                            ...
                                            if (texture != nullptr) {
                                                /*
                                                if (dumpRGBA32) {
                                                    std::wstring pngFilename = FileDialog::getSaveFilename(window, L"PNG (*.png)\0*.png\0\0");
                                                    if (!pngFilename.empty()) {
                                                        const std::string pngFilenameUtf8 = UTF::from16To8(pngFilename);
                                                        stbi_write_png(pngFilenameUtf8.c_str(), texture->width, texture->height, 4, texture->bytesNative.data(), 4 * texture->width);
                                                    }
                                                }
                                                */
                                                if (dumpTMEM) {
                                                    std::filesystem::path binFilename = FileDialog::getSaveFilename(window, L"BIN (*.bin)\0*.bin\0\0");
                                ...
                                if (pixelShaderButton || vertexShaderButton) {
                                    ...
                                    std::filesystem::path shaderFilename = FileDialog::getSaveFilename(window, L"HLSL (*.hlsl)\0*.hlsl\0\0");
