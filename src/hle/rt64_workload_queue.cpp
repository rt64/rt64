    ...
    void WorkloadQueue::renderThreadLoop() {
        ...
        while (threadsRunning) {
            ...
            if (processCursor >= 0) {
                ...
                for (uint32_t f = 0; (f < displayFrames) && !skipFrames; f++) {
                    ...
                    if (generateInterpolatedFrames) {
                        ...
                        if (usingMSAA || (f > 0)) {
                            uint32_t targetIndex = usingMSAA ? f : (f - 1);
                    ...
                    if (generateInterpolatedFrames && (usingMSAA || (f > 0))) {
                        {
                            std::scoped_lock<std::mutex> cursorLock(cursorMutex);
                            skipFrames = ((f + 1) < displayFrames) && (writeCursor != threadCursor);
                        }

                        {
                            std::scoped_lock<std::mutex> managerLock(ext.sharedResources->interpolatedMutex);
                            curFrameCounters.skipped = skipFrames;
                            curFrameCounters.available++;
                        }

                        // Add the amount of display ticks that correspond to the remaining frames.
                        if (skipFrames) {
                            displayTicks += workload.viOriginalRate * (displayFrames - (f + 1));
                        }
